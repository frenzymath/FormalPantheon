...
/-!
# Divisor fibers for the second zero coefficient

This is the symmetric nonzero-product case in the repaired zero-coefficient
cover for `MAYNARD-PRD-PUBLISHED`, Lemma 15.2, pp. 210--211. The key keeps
the second source coordinate and the two coefficients not attached to a
source coordinate; the fiber is recovered from the remaining factor pair.
-/
...
/-- The `v2=0` witness case is controlled by two bounded free coefficients
and one signed factor-pair fiber. -/
theorem zeroSecondCoefficientPlaneWitnesses_card_real_le
  ...
  have hfiber : forall k : Prod (Fin X) (Prod Int Int), k ∈ outer ->
      ...
      · intro w hw
        ...
        have htarget : Not (z = 0) := by
          ...
          have hproduct : w.v 0 * (w.a1.val : Int) = 0 := by
            ...
            exact hrelation
          have ha1 : Not ((w.a1.val : Int) = 0) := by
        ...
        change (w.v 0, (w.a1.val : Int)) ∈ z.divisorsAntidiag
    ...
    · let w := hnonempty.choose
      ...
      have hbound := lineZeroTermTarget_natAbs_le_three_mul_sq
        w.a1 0 (w.v 2) w.v4 hVNonneg hVX (by simpa using hVNonneg)
