...
/-!
# Primes with restricted decimal digits: public statement
...
The count uses standard, unpadded decimal expansions and a strict real cutoff. The four
positive comparison constants are uniform in the digit and cutoff. The equivalence with the
paper's comparison orientation is proved in `Foundations.Comparability`; the closed analytic
proof is in `Proof.MainTheorem`.
-/
