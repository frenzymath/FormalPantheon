import PrimesRestrictedDigits.SieveAsymptotics.SectionSixFirstLowBelowFirstRepeatedBound
import PrimesRestrictedDigits.SieveAsymptotics.SectionSixFirstLowBelowSecondRepeatedBound
...
/-!
# Low-below repeated-sum absorption

The two repeated-current-prime corrections in the exact low-below ledger are jointly absorbed
into every positive logarithmic budget. Each family costs one divisor-candidate factor, so the
private finite combination uses `2 * K`.

Source: MAYNARD-PRD-PUBLISHED, Section 6, pp. 143--144, the repeated-prime displays
immediately before and through Eq. (6.13).
-/
...
private theorem
    sectionSixFirstLowBelowRepeatedSums_abs_le_quarterTieCharge
      ...
      abs (sectionSixFirstLowBelowFirstRepeatedSum
        ...
        (K : Real) *
          sectionSixDirectQuarterTieCharge digit length
            (sectionSixThetaGap epsilon) := by
    simpa only [K] using
      (abs_sectionSixFirstLowBelowFirstRepeatedSum_le_quarterTieCharge
      ...
      abs (sectionSixFirstLowBelowSecondRepeatedSum
        ...
        (K : Real) *
          sectionSixDirectQuarterTieCharge digit length
            (sectionSixThetaGap epsilon) := by
    simpa only [K] using
      (abs_sectionSixFirstLowBelowSecondRepeatedSum_le_quarterTieCharge
        epsilon hepsilon hepsilonSmall digit hlength hfive)
  change
    abs (sectionSixFirstLowBelowFirstRepeatedSum
          epsilon digit length) +
...
/-- The two low-below repeated corrections are jointly smaller than
every positive logarithmic budget, uniformly in the excluded digit. -/
theorem exists_sectionSixFirstLowBelowRepeatedSums_budget_upper
    (epsilon : Real) (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64)
