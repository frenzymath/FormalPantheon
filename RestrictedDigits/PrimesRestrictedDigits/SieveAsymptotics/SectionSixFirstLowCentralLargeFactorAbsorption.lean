...
import PrimesRestrictedDigits.SieveAsymptotics.SectionSixFirstLowCentralLargeFactorIncidence
...
/-!
# Low central-large factor-error absorption

The exact signed factor error is bounded by the two outside-near carriers. The weak Type II
small-product tail and decay of its varying width then absorb that charge into every positive
logarithmic budget.
...
/-- The exact low central-large factor error is controlled by the two common
outside-near carriers, with no incidence loss. -/
theorem abs_sectionSixFirstLowCentralLargeFactorError_le_outsideNear
    {epsilon : Real} (digit : Fin 10)
  ...
  have htailA : tail A ≤ ((A \ near).card : Real) := by
    simpa only [tail, A, near, rho, XNat] using
      (sectionSixFirstLowCentralLargeFactorTailSum_le_outsideNear
  ...
  have herror :
    ...
    simpa only [tail, A, B, lambda, X, XNat, mul_div_assoc] using
      (sectionSixFirstLowCentralLargeFactorError_eq_tailDiscrepancy
        epsilon digit length)
  dsimp only
  rw [herror]
  calc
...
/-- The low central-large factor error is eventually below every positive
logarithmic budget, uniformly in the excluded digit. -/
theorem exists_sectionSixFirstLowCentralLargeFactorError_budget_upper
  ...
  have hfinite :
    ...
    simpa only [A, B, near, lambda, rho, X, XNat] using
      (abs_sectionSixFirstLowCentralLargeFactorError_le_outsideNear
        (epsilon := epsilon) digit hlengthOne
  ...
  change abs (sectionSixFirstLowCentralLargeFactorError
