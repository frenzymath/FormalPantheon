...
/-!
# Finite lower bound for the low central-small quadruple term
...
private theorem sectionSixFirstLowCentralSmallQuadruple_finiteIndexData
...
private theorem
    sectionSixFirstLowCentralSmallQuadruple_buchstabParameter_bounds
    ...
    let X : Real := ((10 ^ length : Nat) : Real)
    let Y : Real := X /
      ((sectionSixFirstLowCentralSmallTripleProduct index.1 * index.2 : Nat) :
        Real)
  ...
  let s : Nat := index.2
  let Y : Real := X /
    ((sectionSixFirstLowCentralSmallTripleProduct index.1 * s : Nat) : Real)
  let tau : Real := Real.log Y / Real.log (s : Real)
  rcases sectionSixFirstLowCentralSmallQuadruple_finiteIndexData
      hindex with
...
private theorem sectionSixFirstLowCentralSmallQuadruple_term_lower
    {C : Real} (hrough :
    ...
    (-densityMass *
        (buchstabFunction
              (Real.log (X /
                ((sectionSixFirstLowCentralSmallTripleProduct index.1 *
  ...
  let r : Nat := index.1.2
  let s : Nat := index.2
  let tripleD : PNat := sectionSixFirstLowCentralSmallTripleModulus index.1
  ...
  have hd : (d : Real) = (p : Real) * q * r * s := by
    ...
    simp only [PNat.mul_coe, Nat.toPNat'_coe,
      sectionSixFirstLowCentralSmallTripleModulus,
...
theorem exists_sectionSixFirstLowCentralSmallQuadrupleFiniteLowerConstant :
    ∃ C : Real, 0 < C ∧
  ...
  have hpoint : ∀ index ∈ indices,
    ...
    simpa only [densityMass, X, indices] using
      sectionSixFirstLowCentralSmallQuadruple_term_lower hrough
        hepsilon hepsilonSmall hlength digit hindex
  dsimp only
  unfold sectionSixFirstLowCentralSmallQuadrupleBuchstabMainSum
    sectionSixFirstLowCentralSmallQuadrupleRoughErrorSum
    sectionSixFirstLowCentralSmallStrictQuadrupleSum
  change -densityMass *
      ((∑ index ∈ indices,
        buchstabFunction
