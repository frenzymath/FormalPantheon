...
/-!
# I6 Buchstab branch range equivalences

This is the endpoint-safe algebraic bridge for Maynard's low-below `R_4`
region (published Section 6, Eq. (6.13)). It locates the argument in the
inverse, middle, and tail ranges only; it does not assert a disjoint partition
or a numerical integral bound.
-/

namespace PrimesRestrictedDigits

private theorem i6_branch_ranges_of_pos {B t : Real} (ht : 0 < t) :
    (3 <= (B - t) / t ↔ 4 * t <= B) ∧
    ((2 <= (B - t) / t ∧ (B - t) / t <= 3) ↔
      (3 * t <= B ∧ B <= 4 * t)) ∧
    ((1 <= (B - t) / t ∧ (B - t) / t <= 2) ↔
...
theorem sectionSixFirstLowBelowQuadruple_branch_ranges
