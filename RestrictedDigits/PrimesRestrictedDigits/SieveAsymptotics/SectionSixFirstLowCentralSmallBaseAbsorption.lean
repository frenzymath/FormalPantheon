...
/-!
# Low central-small base-sum absorption
...
/-- The pair and triple low central-small base sums have a joint arbitrarily
small Proposition 6.1 bound. -/
theorem exists_sectionSixFirstLowCentralSmallBaseSums_budget_upper
  ...
  obtain ⟨pairLength, hpairLength, hpairAt⟩ :=
    propositionSixOneMixed epsilon hepsilon hepsilonSmall hepsilonRosser
      (sectionSixFirstLowCentralSmallPairPresentation epsilon)
      (rho / 2) (by positivity)
  obtain ⟨tripleLength, htripleLength, htripleAt⟩ :=
    propositionSixOneMixed epsilon hepsilon hepsilonSmall hepsilonRosser
      (sectionSixFirstLowCentralSmallTriplePresentation epsilon)
      (rho / 2) (by positivity)
  ...
  rw [← sectionSixFirstLowCentralSmallPairBaseSum_eq_propositionSixOneSum
    epsilon hepsilon hepsilonSmall digit hlengthOne] at hpairBound
  rw [← sectionSixFirstLowCentralSmallTripleBaseSum_eq_propositionSixOneSum
    epsilon hepsilon hepsilonSmall digit hlengthOne] at htripleBound
  calc
    abs (sectionSixFirstLowCentralSmallPairBaseSum epsilon digit length) +
        abs (sectionSixFirstLowCentralSmallTripleBaseSum epsilon digit length) <=
      (rho / 2) *
          ((paddedRestrictedNumbers digit length).card : Real) /
