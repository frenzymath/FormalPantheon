...
/-!
# Buchstab sums for the high central-small strict quadruple term
...
Source: `MAYNARD-PRD-PUBLISHED`, Section 6, pp. 145--146, Eq. (6.16) and region `R5`.
-/
...
private def sectionSixFirstHighCentralSmallQuadrupleNatTuple
    (index : SectionSixFirstHighCentralSmallQuadrupleIndex) :
    ((Nat × Nat) × Nat) × Nat :=
  (((index.1.1.1, index.1.1.2), index.1.2), index.2)

private theorem sectionSixFirstHighCentralSmallQuadrupleNatTuple_injective :
    Function.Injective sectionSixFirstHighCentralSmallQuadrupleNatTuple := by
  rintro ⟨⟨⟨p, q⟩, r⟩, s⟩ ⟨⟨⟨p', q'⟩, r'⟩, s'⟩ h
  simp only [sectionSixFirstHighCentralSmallQuadrupleNatTuple,
    Prod.mk.injEq] at h
  rcases h with ⟨⟨⟨rfl, rfl⟩, rfl⟩, rfl⟩
  rfl

noncomputable def sectionSixFirstHighCentralSmallQuadrupleBuchstabMainSum
...
noncomputable def sectionSixFirstHighCentralSmallQuadrupleRoughErrorSum
    (epsilon : Real) (length : Nat) : Real :=
  let X : Real := ((10 ^ length : Nat) : Real)
  ∑ index ∈ sectionSixFirstHighCentralSmallQuadrupleIndices epsilon length,
...
private theorem sectionSixFirstHighCentralSmallQuadruple_indexData
...
private theorem sectionSixFirstHighCentralSmallQuadruple_roughSummand_le
    {epsilon : Real} (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64)
    {length : Nat} (hlength : 1 <= length)
    {index : SectionSixFirstHighCentralSmallQuadrupleIndex}
  ...
  rcases sectionSixFirstHighCentralSmallQuadruple_indexData
  ...
  have hsY : (s : Real) < Y := by
    ...
    apply (lt_div_iff₀ (mul_pos (mul_pos (mul_pos hpPos hqPos) hrPos)
      hsPos)).2
...
theorem sectionSixFirstHighCentralSmallQuadrupleRoughErrorSum_le
    (epsilon : Real) (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64)
    {length : Nat} (hlength : 1 <= length) :
    sectionSixFirstHighCentralSmallQuadrupleRoughErrorSum epsilon length <=
      (2 * Real.log 4) ^ 4 /
        (sectionSixThetaGap epsilon ^ 6 *
  ...
  have hpoint : ∀ index ∈ indices,
    ...
    simpa only [indices, X, XNat, gap, primeWeight, denominator] using
      sectionSixFirstHighCentralSmallQuadruple_roughSummand_le
        hepsilon hepsilonSmall hlength hindex
  have hsubset :
      indices.image sectionSixFirstHighCentralSmallQuadrupleNatTuple ⊆
    ...
    have hsDvd : s ∣ p * q * r * s * s := by
      ...
      simp only [Nat.mul_comm, Nat.mul_left_comm]
    have hpLe : p <= XNat :=
      (Nat.le_of_dvd hcapPos hpDvd).trans (Nat.le_of_lt hcap)
    have hqLe : q <= XNat :=
      (Nat.le_of_dvd hcapPos hqDvd).trans (Nat.le_of_lt hcap)
    have hrLe : r <= XNat :=
      (Nat.le_of_dvd hcapPos hrDvd).trans (Nat.le_of_lt hcap)
    have hsLe : s <= XNat :=
      (Nat.le_of_dvd hcapPos hsDvd).trans (Nat.le_of_lt hcap)
  ...
  unfold sectionSixFirstHighCentralSmallQuadrupleRoughErrorSum
  ...
  calc
    ...
    _ = (∑ quadruple ∈
      ...
      rw [← Finset.sum_div,
        Finset.sum_image
          sectionSixFirstHighCentralSmallQuadrupleNatTuple_injective.injOn]
