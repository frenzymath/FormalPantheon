...
import PrimesRestrictedDigits.SieveAsymptotics.SourceRosserErrorBudget
...
/-!
# Pointwise corrected source fundamental-sieve estimate

This applies the paired Rosser main sums to the exact decimal source `BoundingSieve` and
inserts the absolute divisor-relative remainder budget.
-/
...
theorem exists_sourceFundamentalSieve_pointwise :
    ∃ C : Real, 1 <= C ∧
        ...
        |((strictSiftedCarrier
              (sieveDilation
                ((paddedRestrictedNumbers digit length).filter
                  (fun n => n.Coprime 10)) outer_d) z).card : Real) -
            (typeIProgressionDensity digit : Real) *
              ((paddedRestrictedNumbers digit length).card : Real) /
                (outer_d : Real) * decimalExcludedPrimeProduct z| <=
          (sourceBoundingSieve digit length outer_d z).totalMass *
            ...
            ∑ e ∈ fundamentalSmoothModuli X epsilon delta,
              |realTypeIProgressionError digit length
                ((outer_d : Nat) * e)| := by
  obtain ⟨C, hC, hMain⟩ :=
    exists_dimensionOneRosserWeakCutoffMainSums
  refine ⟨C, hC, ?_⟩
  intro digit length outer_d X epsilon delta hX hepsilon hepsilonSmall
  ...
  have hmainPair :
    ...
    have h := hMain digit length outer_d
  ...
  have hmass : 0 <= sieve.totalMass := by
    simpa [sieve] using
      sourceBoundingSieve_totalMass_nonneg digit length outer_d (X ^ delta)
  have hpoint :=
  ...
  have hupper := sourceBoundingSieve_errSum_upper_le_fundamental
    digit length outer_d (X := X) (epsilon := epsilon) (delta := delta)
      (by linarith : 1 < X ^ (epsilon / 2))
  have hlower := sourceBoundingSieve_errSum_lower_le_fundamental
    digit length outer_d (X := X) (epsilon := epsilon) (delta := delta)
  ...
  have hsift := sourceBoundingSieve_siftedSum_eq_card digit length outer_d
    (z := X ^ delta) hcutoff
  have hrewritten :=
    hpoint.trans (add_le_add (le_refl
      (sieve.totalMass * (V / s * (C * Real.exp (-s))))) hmax)
  simpa [sieve, V, s, hsift, sourceBoundingSieve_totalMass] using hrewritten
