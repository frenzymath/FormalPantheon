...
/-!
# Rough-error bound for the low-below clean quadruple term

This file bounds the raw rough-error sum by enlarging its exact carrier to a fourfold box of
primes and applying the fourth power of the prime-log mass estimate.
...
private def sectionSixFirstLowBelowQuadrupleNatTuple
    (index : SectionSixFirstLowBelowQuadrupleIndex) :
    ((Nat × Nat) × Nat) × Nat :=
  (((index.1.1.1, index.1.1.2), index.1.2), index.2)

private theorem sectionSixFirstLowBelowQuadrupleNatTuple_injective :
    Function.Injective sectionSixFirstLowBelowQuadrupleNatTuple := by
  rintro ⟨⟨⟨p, q⟩, r⟩, s⟩ ⟨⟨⟨p', q'⟩, r'⟩, s'⟩ h
  simp only [sectionSixFirstLowBelowQuadrupleNatTuple, Prod.mk.injEq] at h
...
private theorem sectionSixFirstLowBelowQuadruple_roughSummand_le
    {epsilon : Real} (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64)
    {length : Nat} (hlength : 1 <= length)
    {index : SectionSixFirstLowBelowQuadrupleIndex}
  ...
  have hlogsThree : (gap * Real.log X) ^ 3 <=
      Real.log (p : Real) * Real.log (q : Real) * Real.log (r : Real) := by
    rw [pow_succ]
...
theorem sectionSixFirstLowBelowQuadrupleRoughErrorSum_le
    (epsilon : Real) (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64)
    {length : Nat} (hlength : 1 <= length) :
    sectionSixFirstLowBelowQuadrupleRoughErrorSum epsilon length <=
      (2 * Real.log 4) ^ 4 /
        (sectionSixThetaGap epsilon ^ 6 *
  ...
  have hpoint : ∀ index ∈ indices,
    ...
    simpa only [indices, X, XNat, gap, primeWeight, denominator] using
      sectionSixFirstLowBelowQuadruple_roughSummand_le
        hepsilon hepsilonSmall hlength hindex
  have hsubset :
      indices.image sectionSixFirstLowBelowQuadrupleNatTuple ⊆
  ...
  unfold sectionSixFirstLowBelowQuadrupleRoughErrorSum
  simp only [sectionSixFirstLowBelowTripleProduct, sectionSixFirstPairProduct]
  ...
  calc
    ...
    _ = (∑ quadruple ∈
          ...
          denominator := by
      rw [← Finset.sum_div,
        Finset.sum_image sectionSixFirstLowBelowQuadrupleNatTuple_injective.injOn]
