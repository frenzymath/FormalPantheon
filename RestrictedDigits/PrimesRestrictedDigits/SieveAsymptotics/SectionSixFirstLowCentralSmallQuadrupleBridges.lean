import PrimesRestrictedDigits.SieveAsymptotics.SectionSixFirstLowCentralSmallQuadrupleCarriers
import PrimesRestrictedDigits.SieveAsymptotics.SectionSixFirstLowCentralSmallQuadrupleBuchstab

/-!
# Finite bridge for the low central-small clean quadruple term
...
Source: `MAYNARD-PRD-PUBLISHED`, Section 6, p. 143, Eq. (6.12), region `R3`.
-/
...
private theorem sectionSixFirstLowCentralSmallQuadrupleBuchstabSummand_eq
  ...
  unfold normalizedPrimeLogWeight
    sectionSixFirstLowCentralSmallQuadrupleKernel normalizedPrimeLog
...
/-- The raw clean quadruple Buchstab sum is its coefficient-one normalized sum. -/
theorem sectionSixFirstLowCentralSmallQuadrupleBuchstabMainSum_eq_normalized
    (epsilon : Real) (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64)
    {length : Nat} (hlength : 1 <= length) :
    sectionSixFirstLowCentralSmallQuadrupleBuchstabMainSum epsilon length =
  ...
  have hpoint : ∀ index ∈ indices,
      buchstabFunction
        ...
        normalizedPrimeLogWeight XNat index.1.1.1 *
          ...
          Real.log X := by
    intro index hindex
    have hquad := mem_sectionSixFirstLowCentralSmallQuadrupleIndices.mp
    ...
    exact sectionSixFirstLowCentralSmallQuadrupleBuchstabSummand_eq hlength
      index (mem_sievePrimeInterval.mp hpqData.1).1
      (mem_sievePrimeInterval.mp hpqData.2).1
      (mem_sievePrimeInterval.mp htriple.2).1
      (mem_sievePrimeInterval.mp hraw.2).1
  unfold sectionSixFirstLowCentralSmallQuadrupleBuchstabMainSum
    normalizedPrimeLogFourfoldSum
  dsimp only [X, XNat, indices]
  rw [← sectionSixFirstLowCentralSmallQuadrupleIndices_map_eq_normalizedPrimeLogFourfoldIndices
