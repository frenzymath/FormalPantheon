...
/-!
# First low-central-small repeated-prime bound
...
private theorem lowCentralSmallPairData
...
private def firstRepeatedPrimeTuple
    (index : SectionSixFirstLowCentralSmallTripleIndex) : Fin 4 -> Nat :=
...
private theorem firstRepeatedData
  ...
  let X : Real := ((10 ^ length : Nat) : Real)
  have htriple := mem_sectionSixFirstLowCentralSmallTripleIndices.mp hindex
  have hpiece := lowCentralSmallPairData htriple.1
...
private theorem firstRepeatedKey_lt_lengthPower
    {epsilon : Real} (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64)
    {length : Nat} (hlength : 1 <= length)
    {index : SectionSixFirstLowCentralSmallTripleIndex}
    (hindex : index ∈ sectionSixFirstLowCentralSmallTripleIndices
      epsilon length) :
    firstRepeatedKey index < 10 ^ length := by
  rw [firstRepeatedKey_eq hepsilon hepsilonSmall hlength hindex]
  let X : Real := ((10 ^ length : Nat) : Real)
  have hdata := firstRepeatedData hepsilon hepsilonSmall hlength hindex
  have htriple := mem_sectionSixFirstLowCentralSmallTripleIndices.mp hindex
  have hpiece := lowCentralSmallPairData htriple.1
  have hpq := mem_sectionSixFirstSecondRepeatedIndices.mp hpiece.1
  have hp := mem_sievePrimeInterval.mp hpq.1
  have hrr : index.2 * index.2 <= index.1.2 * index.1.2 :=
    Nat.mul_le_mul hdata.2.2.2.2.1 hdata.2.2.2.2.1
  have hkey : index.1.1 * index.1.2 * index.2 * index.2 <=
      (index.1.1 * index.1.2 * index.1.2) * index.1.2 := by
    calc
      _ = (index.1.1 * index.1.2) * (index.2 * index.2) := by ring
      _ <= (index.1.1 * index.1.2) *
          (index.1.2 * index.1.2) := Nat.mul_le_mul_left _ hrr
      _ = _ := by ring
  have hqpReal : (index.1.2 : Real) <= index.1.1 := by
    exact_mod_cast hdata.2.2.2.2.2.1
  have hqZTwo : (index.1.2 : Real) <= sectionSixZTwo epsilon X :=
    hqpReal.trans (by simpa only [X] using hp.2.2)
  have hz : sectionSixZSix epsilon X * sectionSixZTwo epsilon X = X := by
    rw [sectionSixZSix, sectionSixZTwo, ← Real.rpow_add (by positivity)]
    have hexponent : (1 - sectionSixThetaOne epsilon) +
        sectionSixThetaOne epsilon = 1 := by ring
    rw [hexponent, Real.rpow_one]
  have hreal :
      ((index.1.1 * index.1.2 * index.2 * index.2 : Nat) : Real) < X := by
    calc
      _ <= (((index.1.1 * index.1.2 * index.1.2) *
          index.1.2 : Nat) : Real) := by exact_mod_cast hkey
      _ = (sectionSixFirstPairSquareProduct index.1 : Real) *
          index.1.2 := by
        norm_num [sectionSixFirstPairSquareProduct]
      _ < sectionSixZSix epsilon X * sectionSixZTwo epsilon X := by
        apply mul_lt_mul
        · simpa only [X] using hpiece.2.2.2
        · exact hqZTwo
        · exact_mod_cast hdata.2.1.pos
        · exact (Real.rpow_pos_of_pos (by positivity) _).le
      _ = X := hz
  dsimp only [X] at hreal
...
private theorem firstRepeatedKey_injOn
    ...
    Set.InjOn firstRepeatedKey
      (sectionSixFirstLowCentralSmallTripleIndices epsilon length :
        Set SectionSixFirstLowCentralSmallTripleIndex) := by
...
private theorem sum_firstRepeatedRepresentedCarrier_le_quarterTie
  ...
  let carrier : SectionSixFirstLowCentralSmallTripleIndex -> Finset Nat :=
...
/-- The first low-central-small repeated residual is absorbed familywise by
the direct quarter-tie charge with multiplicity `2 ^ ceil (1 / gap)`. -/
theorem abs_sectionSixFirstLowCentralSmallFirstRepeatedSum_le_quarterTieCharge
    (epsilon : Real) (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64)
  ...
  unfold sectionSixFirstLowCentralSmallFirstRepeatedSum
