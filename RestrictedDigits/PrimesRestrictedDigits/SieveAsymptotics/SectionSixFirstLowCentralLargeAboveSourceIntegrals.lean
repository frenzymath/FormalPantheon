...
/-!
# The literal source integral for the low central-large above term
...
private theorem sectionSixFirstLowCentralLargeAboveSourceRegion_convex
    (epsilon : Real) :
    Convex Real
      (sectionSixFirstLowCentralLargeAboveSourceRegion epsilon) := by
  have h :=
  ...
  simpa [sectionSixFirstLowCentralLargeAboveSourceRegion, tripleU, tripleV,
    tripleW, triplePair, Set.inter_def, sub_neg] using h

private theorem sectionSixFirstLowCentralLargeAboveSourceRegion_subset
    (epsilon : Real) :
    sectionSixFirstLowCentralLargeAboveSourceRegion epsilon ⊆
      sectionSixFirstLowCentralLargeAboveRegion epsilon := by
  rintro x ⟨hgap, horder, hu, hsumLower, hsumUpper,
    hsquareLower, hsquareUpper, hvw, hcap, habove⟩
  exact ⟨hgap, horder.le, hu.le, hsumLower, hsumUpper,
    hsquareLower.le, hsquareUpper.le, hvw, hcap.le, habove⟩
...
private theorem sectionSixFirstLowCentralLargeAboveRegion_subset_closure
    (epsilon : Real) (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64) :
    sectionSixFirstLowCentralLargeAboveRegion epsilon ⊆
      closure (sectionSixFirstLowCentralLargeAboveSourceRegion epsilon) := by
  let y : (Real × Real) × Real := ((33 / 100, 1 / 5), 23 / 100)
  have hy : y ∈
      sectionSixFirstLowCentralLargeAboveSourceRegion epsilon := by
    simp only [sectionSixFirstLowCentralLargeAboveSourceRegion, mem_setOf_eq]
  ...
  rcases hx with ⟨hxgap, hxorder, hxupper, hxsumLower, hxsumUpper,
    hxsquareLower, hxsquareUpper, hxvw, hxcap, hxabove⟩
  rcases hy with ⟨hygap, hyorder, hyupper, hysumLower, hysumUpper,
    hysquareLower, hysquareUpper, hyvw, hycap, hyabove⟩
  ...
  · nlinarith [combo_lt_combo ha hb hxabove.le hyabove]

local instance sectionSixFirstLowCentralLargeAboveSourcePairHaar :
    Measure.IsAddHaarMeasure (volume : Measure (Real × Real)) :=
  Measure.prod.instIsAddHaarMeasure volume volume

local instance sectionSixFirstLowCentralLargeAboveSourceTripleHaar :
...
theorem sectionSixFirstLowCentralLargeAboveIntegral_eq_source
    (epsilon : Real) (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64) :
    sectionSixFirstLowCentralLargeAboveIntegral epsilon =
      ∫ x in sectionSixFirstLowCentralLargeAboveSourceRegion epsilon,
        sectionSixFirstLowCentralLargeAboveKernel x := by
  unfold sectionSixFirstLowCentralLargeAboveIntegral
  apply setIntegral_congr_set
  exact ae_eq_of_subset_closure
    (sectionSixFirstLowCentralLargeAboveSourceRegion_subset epsilon)
    (sectionSixFirstLowCentralLargeAboveRegion_subset_closure epsilon
      hepsilon hepsilonSmall)
    (sectionSixFirstLowCentralLargeAboveSourceRegion_convex epsilon)
