...
/-!
# Low-below second repeated bound

The complete second repeated key on the ordinary raw nested quadruple carrier with final
interval `(z1,r]` is charged, with familywise multiplicity, to the common quarter-tie carrier.

Source: `MAYNARD-PRD-PUBLISHED`, Section 6, pp. 143--144, the repeated displays before and
through Eq. (6.13).
-/
...
private theorem secondData
    ...
    (hindex : index ∈ sectionSixFirstLowBelowRawQuadrupleIndices
      epsilon length) :
    ...
    index.1.1.1.Prime ∧ index.1.1.2.Prime ∧ index.1.2.Prime ∧
      ...
      weakRoughPredicate (X ^ sectionSixThetaGap epsilon)
        (sectionSixFirstLowBelowTripleModulus index.1 : Nat) := by
  let X : Real := ((10 ^ length : Nat) : Real)
  have hquad := mem_sectionSixFirstLowBelowRawQuadrupleIndices.mp hindex
  have htriple := mem_sectionSixFirstLowBelowTripleIndices.mp hquad.1
  ...
  have hdRough : weakRoughPredicate (X ^ sectionSixThetaGap epsilon)
      (sectionSixFirstLowBelowTripleModulus index.1 : Nat) := by
    rw [sectionSixFirstLowBelowTripleModulus_coe hp.1 hq.1 hr.1]
...
private theorem secondKey_eq
  ...
  rw [sectionSixFirstRepeatedKey_eq _ h.2.2.2.1,
    sectionSixFirstLowBelowTripleModulus_coe h.1 h.2.1 h.2.2.1]
  simp only [sectionSixFirstLowBelowTripleProduct,
    sectionSixFirstPairProduct]
...
private theorem secondKey_injOn
    ...
    (length : Nat) (hlength : 1 <= length) :
    Set.InjOn secondKey
      (sectionSixFirstLowBelowRawQuadrupleIndices epsilon length :
        Set SectionSixFirstLowBelowQuadrupleIndex) := by
...
private theorem sum_second_le
    ...
    (∑ index ∈ sectionSixFirstLowBelowRawQuadrupleIndices epsilon length,
      ((sectionSixFirstRepeatedRepresentedCarrier C
        (sectionSixFirstLowBelowTripleModulus index.1)
        index.2).card : Real)) <=
      (K : Real) *
  ...
  let X : Real := ((10 ^ length : Nat) : Real)
  let gap : Real := sectionSixThetaGap epsilon
  let states := sectionSixFirstLowBelowRawQuadrupleIndices epsilon length
  let carrier : SectionSixFirstLowBelowQuadrupleIndex -> Finset Nat :=
...
/-- The low-below ordinary raw nested second repeated family, with final
interval `(z1,r]`, costs one divisor-candidate factor times the common
quarter-tie charge. -/
theorem abs_sectionSixFirstLowBelowSecondRepeatedSum_le_quarterTieCharge
    (epsilon : Real) (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64)
  ...
  unfold sectionSixFirstLowBelowSecondRepeatedSum
  apply abs_sectionSixFirstRepeatedIndexSum_le_quarterTieCharge
    digit length gap K states
      (fun index => sectionSixFirstLowBelowTripleModulus index.1)
      (fun index => index.2)
  · intro index hindex
    exact (mem_sievePrimeInterval.mp
      (mem_sectionSixFirstLowBelowRawQuadrupleIndices.mp hindex).2).1
