import PrimesRestrictedDigits.SieveAsymptotics.SectionSixFirstHighCentralSmallQuadrupleSourceIntegrals
import PrimesRestrictedDigits.SieveAsymptotics.SectionSixFirstHighCentralSmallQuadrupleBridges
import PrimesRestrictedDigits.SieveAsymptotics.SectionSixFirstHighCentralSmallQuadrupleFiniteLower

/-!
# Eventual lower bound for the high central-small quadruple term

The finite strict-rough estimate is combined with normalized prime-log fourfold convergence
and the fourfold log-prime error bound. This is Maynard's `I_9` contribution, with coefficient
one and the full strict sifted term.
...
private theorem sectionSixFirstHighCentralSmallQuadruple_lower_of_bounds
    ...
    (hnormalized : normalized <=
      sectionSixFirstHighCentralSmallQuadrupleIntegral epsilon + rho / 2)
    (herrorSmall :
      C * (2 * Real.log 4) ^ 4 /
  ...
  have herror :=
    sectionSixFirstHighCentralSmallQuadrupleRoughErrorSum_le
    ...
    calc
      normalized / Real.log X +
        ...
        add_le_add (div_le_div_of_nonneg_right hnormalized hlogX.le)
          herrorScaled
      _ = (sectionSixFirstHighCentralSmallQuadrupleIntegral epsilon + rho) /
          Real.log X := by ring
  dsimp only
  ...
  calc
    ...
    _ <= sectionSixFirstHighCentralSmallStrictQuadrupleSum
          epsilon digit length := by
      simpa only [densityMass] using hfinite

theorem exists_sectionSixFirstHighCentralSmallStrictQuadrupleSum_lower
        ...
        ∀ digit : Fin 10,
          ...
          (-scale *
            ...
            sectionSixFirstHighCentralSmallStrictQuadrupleSum
              epsilon digit length) := by
  obtain ⟨C, hC, hfinite⟩ :=
    exists_sectionSixFirstHighCentralSmallQuadrupleFiniteLowerConstant
  ...
  have hsumTendsto : Tendsto normalizedSum atTop
    ...
    simpa only [normalizedSum] using
      tendsto_sectionSixFirstHighCentralSmallQuadrupleKernelSum
  ...
  have herrorAt' :
    ...
    ring
  dsimp only
  exact sectionSixFirstHighCentralSmallQuadruple_lower_of_bounds hC.le
    hepsilon hepsilonSmall hlengthOne digit
    (sectionSixFirstHighCentralSmallQuadrupleBuchstabMainSum_eq_normalized
