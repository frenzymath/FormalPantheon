...
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C100S007
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C100S012

/-!
# exact I6 chamber certificate C100S013
-/
...
def tree : RationalTetraClipD1002 16 Rat :=
  .split 2 (340381137 / 650072653) C100S007.tree C100S012.tree
...
theorem valid :
  ...
  exact valid_split (by decide +kernel) (by decide +kernel)
    (by simpa only [C100S007.root_eq_path] using C100S007.valid)
    (by simpa only [C100S012.root_eq_path] using C100S012.valid)

theorem replay :
    tree.replayWeightRatD1010 (i6D1005ChamberWalls label 3 0) root = (109058364643 / 20000000000000) := by
  rw [root_eq_path]
  have h0 : C100S007.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 3 0)
      (let T := i6D999OrderedPairRoot;
       ...
       T) = (179794392473 / 50000000000000) := by
    simpa only [C100S007.root_eq_path] using C100S007.replay
  have h1 : C100S012.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 3 0)
      (let T := i6D999OrderedPairRoot;
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 2).1 (T.edgePointD1000 2 (2 / 3));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 5).1 (T.edgePointD1000 5 (1 / 2));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 2).1 (T.edgePointD1000 2 (758 / 24395));
       ...
       T) = (185703038269 / 100000000000000) := by
    simpa only [C100S012.root_eq_path] using C100S012.replay
  calc
    _ = ((179794392473 / 50000000000000) : Rat) + (185703038269 / 100000000000000) :=
      replay_split h0 h1
