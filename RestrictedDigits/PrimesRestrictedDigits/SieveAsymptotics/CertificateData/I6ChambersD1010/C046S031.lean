...
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C046S001
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C046S030

/-!
# exact I6 chamber certificate C046S031
-/
...
def tree : RationalTetraClipD1002 16 Rat :=
  .split 1 (82507 / 115005) C046S001.tree C046S030.tree
...
theorem valid :
  ...
  exact valid_split (by decide +kernel) (by decide +kernel)
    (by simpa only [C046S001.root_eq_path] using C046S001.valid)
    (by simpa only [C046S030.root_eq_path] using C046S030.valid)

theorem replay :
    tree.replayWeightRatD1010 (i6D1005ChamberWalls label 0 0) root = (1173758807207 / 50000000000000) := by
  rw [root_eq_path]
  have h0 : C046S001.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 0 0)
      (let T := i6D999OrderedPairRoot;
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 0).2 (T.edgePointD1000 0 (82507 / 115005));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 1).1 (T.edgePointD1000 1 (82507 / 115005));
       T) = 0 := by
    simpa only [C046S001.root_eq_path] using C046S001.replay
  have h1 : C046S030.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 0 0)
      (let T := i6D999OrderedPairRoot;
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 0).2 (T.edgePointD1000 0 (82507 / 115005));
    ...
    simpa only [C046S030.root_eq_path] using C046S030.replay
  calc
