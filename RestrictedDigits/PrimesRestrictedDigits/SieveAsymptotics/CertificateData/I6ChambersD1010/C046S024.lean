...
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C046S020
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C046S023

/-!
# exact I6 chamber certificate C046S024
-/
...
def tree : RationalTetraClipD1002 16 Rat :=
  .split 0 (1 / 2) C046S020.tree C046S023.tree
...
theorem valid :
  ...
  exact valid_split (by decide +kernel) (by decide +kernel)
    (by simpa only [C046S020.root_eq_path] using C046S020.valid)
    (by simpa only [C046S023.root_eq_path] using C046S023.valid)

theorem replay :
    tree.replayWeightRatD1010 (i6D1005ChamberWalls label 0 0) root = (722510629471 / 100000000000000) := by
  rw [root_eq_path]
  have h0 : C046S020.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 0 0)
      (let T := i6D999OrderedPairRoot;
       ...
       T) = (42442594663 / 12500000000000) := by
    simpa only [C046S020.root_eq_path] using C046S020.replay
  have h1 : C046S023.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 0 0)
      (let T := i6D999OrderedPairRoot;
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 0).2 (T.edgePointD1000 0 (82507 / 115005));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 1).2 (T.edgePointD1000 1 (82507 / 115005));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 2).2 (T.edgePointD1000 2 (165014 / 345015));
       ...
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 1).2 (T.edgePointD1000 1 (1 / 2));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 0).2 (T.edgePointD1000 0 (1 / 2));
       T) = (382969872167 / 100000000000000) := by
    simpa only [C046S023.root_eq_path] using C046S023.replay
  calc
    _ = ((42442594663 / 12500000000000) : Rat) + (382969872167 / 100000000000000) :=
      replay_split h0 h1
