...
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C100S010
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C100S011

/-!
# exact I6 chamber certificate C100S012
-/
...
def tree : RationalTetraClipD1002 16 Rat :=
  .split 0 (1 / 2) C100S010.tree C100S011.tree
...
theorem valid :
  ...
  exact valid_split (by decide +kernel) (by decide +kernel)
    (by simpa only [C100S010.root_eq_path] using C100S010.valid)
    (by simpa only [C100S011.root_eq_path] using C100S011.valid)

theorem replay :
    tree.replayWeightRatD1010 (i6D1005ChamberWalls label 3 0) root = (185703038269 / 100000000000000) := by
  rw [root_eq_path]
  have h0 : C100S010.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 3 0)
      (let T := i6D999OrderedPairRoot;
       ...
       T) = (6640457789 / 6250000000000) := by
    simpa only [C100S010.root_eq_path] using C100S010.replay
  have h1 : C100S011.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 3 0)
      (let T := i6D999OrderedPairRoot;
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 2).1 (T.edgePointD1000 2 (2 / 3));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 5).1 (T.edgePointD1000 5 (1 / 2));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 2).1 (T.edgePointD1000 2 (758 / 24395));
       ...
       T) = (15891142729 / 20000000000000) := by
    simpa only [C100S011.root_eq_path] using C100S011.replay
  calc
    _ = ((6640457789 / 6250000000000) : Rat) + (15891142729 / 20000000000000) :=
      replay_split h0 h1
