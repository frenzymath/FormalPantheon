...
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C100S004
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C100S005

/-!
# exact I6 chamber certificate C100S006
-/
...
def tree : RationalTetraClipD1002 16 Rat :=
  .split 3 (1 / 2) C100S004.tree C100S005.tree
...
theorem valid :
  ...
  exact valid_split (by decide +kernel) (by decide +kernel)
    (by simpa only [C100S004.root_eq_path] using C100S004.valid)
    (by simpa only [C100S005.root_eq_path] using C100S005.valid)

theorem replay :
    tree.replayWeightRatD1010 (i6D1005ChamberWalls label 3 0) root = (35255565499 / 10000000000000) := by
  rw [root_eq_path]
  have h0 : C100S004.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 3 0)
      (let T := i6D999OrderedPairRoot;
       ...
       T) = (71756209403 / 50000000000000) := by
    simpa only [C100S004.root_eq_path] using C100S004.replay
  have h1 : C100S005.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 3 0)
      (let T := i6D999OrderedPairRoot;
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 2).1 (T.edgePointD1000 2 (2 / 3));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 5).1 (T.edgePointD1000 5 (1 / 2));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 2).1 (T.edgePointD1000 2 (758 / 24395));
       ...
       T) = (26130404523 / 12500000000000) := by
    simpa only [C100S005.root_eq_path] using C100S005.replay
  calc
    _ = ((71756209403 / 50000000000000) : Rat) + (26130404523 / 12500000000000) :=
      replay_split h0 h1
