...
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C059S023
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C059S036

/-!
# exact I6 chamber certificate C059S037
-/
...
def tree : RationalTetraClipD1002 16 Rat :=
  .split 0 (1 / 2) C059S023.tree C059S036.tree
...
theorem valid :
  ...
  exact valid_split (by decide +kernel) (by decide +kernel)
    (by simpa only [C059S023.root_eq_path] using C059S023.valid)
    (by simpa only [C059S036.root_eq_path] using C059S036.valid)

theorem replay :
    tree.replayWeightRatD1010 (i6D1005ChamberWalls label 5 0) root = (236601999279 / 10000000000000) := by
  rw [root_eq_path]
  have h0 : C059S023.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 5 0)
      (let T := i6D999OrderedPairRoot;
       ...
       T) = (326176672089 / 25000000000000) := by
    simpa only [C059S023.root_eq_path] using C059S023.replay
  have h1 : C059S036.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 5 0)
      (let T := i6D999OrderedPairRoot;
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 0).2 (T.edgePointD1000 0 (82507 / 115005));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 1).2 (T.edgePointD1000 1 (82507 / 115005));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 2).2 (T.edgePointD1000 2 (165014 / 345015));
       ...
       T) = (530656652217 / 50000000000000) := by
    simpa only [C059S036.root_eq_path] using C059S036.replay
  calc
    _ = ((326176672089 / 25000000000000) : Rat) + (530656652217 / 50000000000000) :=
      replay_split h0 h1
