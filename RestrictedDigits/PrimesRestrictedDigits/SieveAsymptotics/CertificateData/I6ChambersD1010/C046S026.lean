...
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C046S016
import PrimesRestrictedDigits.SieveAsymptotics.CertificateData.I6ChambersD1010.C046S025

/-!
# exact I6 chamber certificate C046S026
-/
...
def tree : RationalTetraClipD1002 16 Rat :=
  .split 0 (1 / 2) C046S016.tree C046S025.tree
...
theorem valid :
  ...
  exact valid_split (by decide +kernel) (by decide +kernel)
    (by simpa only [C046S016.root_eq_path] using C046S016.valid)
    (by simpa only [C046S025.root_eq_path] using C046S025.valid)

theorem replay :
    tree.replayWeightRatD1010 (i6D1005ChamberWalls label 0 0) root = (1173758807207 / 50000000000000) := by
  rw [root_eq_path]
  have h0 : C046S016.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 0 0)
      (let T := i6D999OrderedPairRoot;
       ...
       T) = (105403594743 / 10000000000000) := by
    simpa only [C046S016.root_eq_path] using C046S016.replay
  have h1 : C046S025.tree.replayWeightRatD1010 (i6D1005ChamberWalls label 0 0)
      (let T := i6D999OrderedPairRoot;
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 0).2 (T.edgePointD1000 0 (82507 / 115005));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 1).2 (T.edgePointD1000 1 (82507 / 115005));
       let T := T.replaceVertex (RationalTetrahedron.edgeEndpoints 2).2 (T.edgePointD1000 2 (165014 / 345015));
       ...
       T) = (161685208373 / 12500000000000) := by
    simpa only [C046S025.root_eq_path] using C046S025.replay
  calc
    _ = ((105403594743 / 10000000000000) : Rat) + (161685208373 / 12500000000000) :=
      replay_split h0 h1
