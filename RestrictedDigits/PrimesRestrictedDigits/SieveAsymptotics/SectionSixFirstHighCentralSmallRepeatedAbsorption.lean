import PrimesRestrictedDigits.SieveAsymptotics.SectionSixFirstHighCentralSmallFirstRepeatedBound
import PrimesRestrictedDigits.SieveAsymptotics.SectionSixFirstHighCentralSmallSecondRepeatedBound
...
/-!
# High central-small repeated-sum absorption

The two repeated-current-prime corrections omitted before Eq. (6.16) are jointly absorbed into
every positive logarithmic budget. Each family costs one divisor-candidate factor, so the
...
private theorem
    sectionSixFirstHighCentralSmallRepeatedSums_abs_le_quarterTieCharge
      ...
      abs (sectionSixFirstHighCentralSmallFirstRepeatedSum
        ...
        (K : Real) *
          sectionSixDirectQuarterTieCharge digit length
            (sectionSixThetaGap epsilon) := by
    simpa only [K] using
      (abs_sectionSixFirstHighCentralSmallFirstRepeatedSum_le_quarterTieCharge
      ...
      abs (sectionSixFirstHighCentralSmallSecondRepeatedSum
        ...
        (K : Real) *
          sectionSixDirectQuarterTieCharge digit length
            (sectionSixThetaGap epsilon) := by
    simpa only [K] using
      (abs_sectionSixFirstHighCentralSmallSecondRepeatedSum_le_quarterTieCharge
        epsilon hepsilon hepsilonSmall digit hlength hfive)
  change
    abs (sectionSixFirstHighCentralSmallFirstRepeatedSum
          epsilon digit length) +
...
/-- The two high central-small repeated corrections are jointly smaller than
every positive logarithmic budget, uniformly in the excluded digit. -/
theorem exists_sectionSixFirstHighCentralSmallRepeatedSums_budget_upper
    (epsilon : Real) (hepsilon : 0 < epsilon)
    (hepsilonSmall : epsilon <= 1 / 64)
