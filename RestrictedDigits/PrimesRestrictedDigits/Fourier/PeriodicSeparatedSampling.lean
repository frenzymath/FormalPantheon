...
/-!
# Separated sampling of a periodic function

This is the generic analytic engine for the repaired large-sieve estimates in
`MAYNARD-PRD-PUBLISHED`, Lemma 10.5.
-/
...
/-- A finite `1 / L`-separated family on the unit circle satisfies the
closed-window sampling bound with factor eight. -/
theorem sum_closedWindowMaximum_le_of_pairwise_circleDist
  ...
  let center (i : ι) : UnitAddCircle := (base i + beta + eta i : Real)
  ...
  have hliftEnvelope0 (z : UnitAddCircle) : 0 <= liftEnvelope z := by
    ...
    exact henvelope0 z
  let radius : Real := 1 / (4 * L)
  let overlap : Real := 4 * (1 + delta * L)
  have hoverlap (z : UnitAddCircle) :
      ((indicesInClosedBall s center z radius).card : Real) <= overlap := by
    exact card_filter_mem_closedBall_le_of_pairwise_dist s shiftedBase center
      hL hdelta hshiftedSeparated hperturbed z
  have hsumF :
  ...
  have hlocal (i : ι) :
      f (base i + beta + eta i) <=
        2 * L * (∫ z in Metric.closedBall (center i) radius, liftF z) +
    ...
    simpa only [center, radius, liftF, liftEnvelope] using
      le_periodicLift_closedBall_average_add_envelope hf henvelope
        hfPeriod henvelopePeriod henvelope0 hvariation hL
          (base i + beta + eta i)
  have hperiodF0 : 0 <= ∫ z : UnitAddCircle, liftF z :=
    MeasureTheory.integral_nonneg hliftF0
  have hperiodEnvelope0 : 0 <= ∫ z : UnitAddCircle, liftEnvelope z :=
    MeasureTheory.integral_nonneg hliftEnvelope0
  ...
  calc
    ...
    _ = overlap *
        (2 * L * (∫ z : UnitAddCircle, liftF z) +
          ∫ z : UnitAddCircle, liftEnvelope z) := by ring
    _ <= overlap *
        (2 * (L * (∫ z : UnitAddCircle, liftF z) +
          ∫ z : UnitAddCircle, liftEnvelope z)) := by
      apply mul_le_mul_of_nonneg_left _ hoverlap0
      nlinarith
    _ = 8 * (1 + delta * L) *
        (L * (∫ z : UnitAddCircle, liftF z) +
          ∫ z : UnitAddCircle, liftEnvelope z) := by
      dsimp [overlap]
      ring
    _ = 8 * (1 + delta * L) *
