...
/-!
# First-orientation fixed-cell reindexing

This file maps each selected pair in a first-orientation scale cell injectively into the
standard and exceptional weighted carriers. The maps retain the aligned grid integers, which
recover both original frequencies.
-/
...
/-- The injective standard target attached to a first-orientation cell pair. -/
noncomputable def latticeFirstStandardCellMap
  ...
  let firstTerm := signedHybridTerm length (key.errorScale (N * K)) m1
    pair.val.val.1 f.b1Prime hm1 hcop1 herror1
  let secondTerm := signedHybridTerm length (key.errorScale (N * K)) m2
    pair.val.val.2 f.b2Prime hm2 hcop2 herror2
  exact ⟨qPrime, (⟨g1Prime, secondTerm⟩, ⟨g2, firstTerm⟩)⟩

theorem latticeFirstStandardCellMap_firstGrid
    (digit : Fin 10) {length : Nat} (N K delta : Real)
    (hN : 1 <= N) (hK : 1 <= K) (hdelta : 0 < delta)
    (hdeltaLower : N / ((10 ^ length : Nat) : Real) <= delta)
    (key : LatticeDecompositionScaleKey length) (d0 d1 : Nat)
    (a : {a // a ∈ latticeFirstScaleCell digit N K delta hN hK hdelta
      hdeltaLower key d0 d1}) :
    (latticeFirstStandardCellMap digit N K delta hN hK hdelta hdeltaLower
      ...
      signedAlignedGridNumerator length a.val.val.val.1
        ...
        (d0 * ((latticeFirstOrientedFactorization N K delta hN hK hdelta
          hdeltaLower a.val).qPrime *
            (latticeFirstOrientedFactorization N K delta hN hK hdelta
              hdeltaLower a.val).g2)) := by
  rfl

theorem latticeFirstStandardCellMap_secondGrid
    (digit : Fin 10) {length : Nat} (N K delta : Real)
    (hN : 1 <= N) (hK : 1 <= K) (hdelta : 0 < delta)
    (hdeltaLower : N / ((10 ^ length : Nat) : Real) <= delta)
    (key : LatticeDecompositionScaleKey length) (d0 d1 : Nat)
...
theorem latticeFirstStandardCellMap_weight
    (digit : Fin 10) {length : Nat} (N K delta : Real)
    (hN : 1 <= N) (hK : 1 <= K) (hdelta : 0 < delta)
    (hdeltaLower : N / ((10 ^ length : Nat) : Real) <= delta)
    (key : LatticeDecompositionScaleKey length) (d0 d1 : Nat)
    ...
    latticeGeneratingPairWeight digit length a.val.val.val =
      latticeStandardCellTargetWeight digit length d0 d1 key.qPrimeScale
        ...
        (latticeFirstStandardCellMap digit N K delta hN hK hdelta
          hdeltaLower key d0 d1 a) := by
  simp only [latticeFirstStandardCellMap, latticeStandardCellTargetWeight,
    latticeSOneCellTermWeight, latticeSTwoCellTermWeight,
    latticeHybridTermWeight_signedHybridTerm, latticeGeneratingPairWeight]
  ring

theorem latticeFirstStandardCellMap_injective
    (digit : Fin 10) {length : Nat} (N K delta : Real)
    (hN : 1 <= N) (hK : 1 <= K) (hdelta : 0 < delta)
    (hdeltaLower : N / ((10 ^ length : Nat) : Real) <= delta)
    (key : LatticeDecompositionScaleKey length) (d0 d1 : Nat) :
  ...
  rw [latticeFirstStandardCellMap_firstGrid,
    latticeFirstStandardCellMap_firstGrid] at hfirstGrid
  rw [latticeFirstStandardCellMap_secondGrid,
    latticeFirstStandardCellMap_secondGrid] at hsecondGrid
...
/-- The first-orientation cell is bounded by the standard `S1*S2` product. -/
theorem latticeFirstScaleCellMass_le_standard
    (digit : Fin 10) {length : Nat} (N K delta : Real)
    (hN : 1 <= N) (hK : 1 <= K) (hdelta : 0 < delta)
    (hdeltaLower : N / ((10 ^ length : Nat) : Real) <= delta)
    (key : LatticeDecompositionScaleKey length) (d0 d1 : Nat) :
  ...
  let reindex := latticeFirstStandardCellMap digit N K delta hN hK hdelta
    hdeltaLower key d0 d1
  calc
    (∑ a ∈ source, latticeGeneratingPairWeight digit length a.val.val) <=
      ...
      exact finset_sum_le_fintype_sum_of_injective source
        ...
        (latticeFirstStandardCellMap_injective digit N K delta hN hK hdelta
          hdeltaLower key d0 d1)
        (latticeFirstStandardCellMap_weight digit N K delta hN hK hdelta
          hdeltaLower key d0 d1)
