...
/-!
# Local Euler factors as formal power series

This file constructs the geometric local factor attached to a positive degree
`e` and a weight `z`.  Its inverse and logarithmic derivative identities are
proved over an arbitrary commutative ring, without division by `e`.
-/
...
/-- The geometric local Euler factor `(1 - z X ^ e)⁻¹`, constructed by
rescaling the geometric series and substituting `X ^ e`. -/
...
/-- The coefficient of `X ^ n` in a positive-degree local Euler factor. -/
@[simp]
...
/-- The local Euler factor is inverse to `1 - z X ^ e`. -/
...
/-- The division-free logarithmic derivative identity for a local Euler
factor: `X L' = e L (L - 1)`. -/
theorem X_mul_derivativeFun_localEuler (z : R) {e : Nat} (he : e ≠ 0) :
    PowerSeries.X * PowerSeries.derivativeFun (localEuler e z) =
  ...
  have hDer :
      L * PowerSeries.derivativeFun Q +
          Q * PowerSeries.derivativeFun L = 0 := by
    have h := congrArg PowerSeries.derivativeFun hInv
    rw [PowerSeries.derivativeFun_mul,
      PowerSeries.derivativeFun_one] at h
    simpa only [smul_eq_mul, add_comm] using h
  have hQDer :
      PowerSeries.derivativeFun Q =
        -PowerSeries.C ((e : R) * z) *
          PowerSeries.X ^ (e - 1) :=
  ...
  linear_combination
    (PowerSeries.X * L) * hDer -
      (PowerSeries.X * PowerSeries.derivativeFun L +
