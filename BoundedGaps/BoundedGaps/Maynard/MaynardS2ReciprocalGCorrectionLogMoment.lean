...
/-!
# Logarithmic moment of the reciprocal-g correction

The quarter-power Euler majorant supplies a uniform logarithmic first moment
for the correction attached to Maynard2013v3, Section 6. See `SEM-355`.
-/
...
private noncomputable def maynardS2ReciprocalGCorrectionQuarterTerm
    (W n : ℕ) : ℝ :=
  |maynardS2ReciprocalGCorrectionAF W n| * (n : ℝ) ^ (1 / 4 : ℝ)

private theorem maynardS2ReciprocalGCorrectionQuarterTerm_mul
    (W : ℕ) {m n : ℕ} (hmn : Nat.Coprime m n) :
    maynardS2ReciprocalGCorrectionQuarterTerm W (m * n) =
      maynardS2ReciprocalGCorrectionQuarterTerm W m *
        maynardS2ReciprocalGCorrectionQuarterTerm W n := by
  unfold maynardS2ReciprocalGCorrectionQuarterTerm
  rw [(maynardS2ReciprocalGCorrectionAF_isMultiplicative W)
      |>.map_mul_of_coprime hmn,
    abs_mul, Nat.cast_mul, Real.mul_rpow (by positivity) (by positivity)]
  ring

private theorem maynardS2ReciprocalGCorrection_weight_le_four_div_sq
...
private theorem maynardS2ReciprocalGCorrection_local_quarter_tsum_le
    (W : ℕ) {p : ℕ} (hp : p.Prime) :
    (∑' i : ℕ,
        maynardS2ReciprocalGCorrectionQuarterTerm W (p ^ i)) ≤
      1 + 20 / (p : ℝ) ^ (3 / 2 : ℝ) := by
  ...
  · subst p
    ...
    · rw [if_neg hW]
      ...
      have hratio : (1 : ℝ) ≤ 20 / (2 : ℝ) ^ (3 / 2 : ℝ) := by
        rw [le_div_iff₀ hpos]
  ...
  · have hp2 : 2 ≤ p := hp.two_le
    ...
    · rw [if_neg hpW, if_neg hpW]
      ...
      simp only [neg_neg]
      have hweight :=
        maynardS2ReciprocalGCorrection_weight_le_four_div_sq hp hp3
      have hpow : (p : ℝ) ^ (1 / 2 : ℝ) *
          (p : ℝ) ^ (3 / 2 : ℝ) = (p : ℝ) ^ 2 := by
      ...
      have hfirst :
          ((2 : ℝ) / ((p : ℝ) * ((p : ℝ) - 2))) *
              (p : ℝ) ^ (1 / 4 : ℝ) ≤
            3 * primeTotientSquareWeight p *
              (p : ℝ) ^ (1 / 2 : ℝ) := by
        calc
          _ ≤ (3 * primeTotientSquareWeight p) *
              (p : ℝ) ^ (1 / 4 : ℝ) :=
      ...
      calc
        1 + ((2 : ℝ) / ((p : ℝ) * ((p : ℝ) - 2))) *
            ...
            ((1 : ℝ) / ((p : ℝ) * ((p : ℝ) - 2))) *
              (p : ℝ) ^ (1 / 2 : ℝ) ≤
            1 + 3 * primeTotientSquareWeight p *
                (p : ℝ) ^ (1 / 2 : ℝ) +
              2 * primeTotientSquareWeight p *
                (p : ℝ) ^ (1 / 2 : ℝ) := by
          linarith
        _ = 1 + 5 * primeTotientSquareWeight p *
              (p : ℝ) ^ (1 / 2 : ℝ) := by ring
        _ ≤ 1 + 5 * (4 / (p : ℝ) ^ 2) *
              (p : ℝ) ^ (1 / 2 : ℝ) := by
        ...
        _ = 1 + 20 / (p : ℝ) ^ (3 / 2 : ℝ) := by
          ...
          rw [show 5 * (4 / (p : ℝ) ^ 2) *
              (p : ℝ) ^ (1 / 2 : ℝ) =
                5 * ((4 / (p : ℝ) ^ 2) *
                  (p : ℝ) ^ (1 / 2 : ℝ)) by ring, hfour]
          ring

noncomputable def maynardS2ReciprocalGCorrectionQuarterConstant : ℝ :=
  Real.exp (∑' n : ℕ, 20 / (n : ℝ) ^ (3 / 2 : ℝ))

private theorem maynardS2ReciprocalGCorrection_quarter_majorant_summable :
    Summable (fun n : ℕ => 20 / (n : ℝ) ^ (3 / 2 : ℝ)) := by
  simpa [div_eq_mul_inv] using
    ((Real.summable_one_div_nat_rpow
      (p := (3 / 2 : ℝ))).mpr (by norm_num)).mul_left 20

private theorem maynardS2ReciprocalGCorrection_local_quarter_product_le
    (W N : ℕ) :
    (∏ p ∈ N.primesBelow,
        ∑' i : ℕ,
          maynardS2ReciprocalGCorrectionQuarterTerm W (p ^ i)) ≤
      maynardS2ReciprocalGCorrectionQuarterConstant := by
  calc
    (∏ p ∈ N.primesBelow,
        ∑' i : ℕ,
          maynardS2ReciprocalGCorrectionQuarterTerm W (p ^ i)) ≤
        ∏ p ∈ N.primesBelow, (1 + 20 / (p : ℝ) ^ (3 / 2 : ℝ)) := by
      ...
      · intro p hp
        exact tsum_nonneg fun i => by
          ...
          positivity
      · intro p hp
        exact maynardS2ReciprocalGCorrection_local_quarter_tsum_le W
          (Nat.prime_of_mem_primesBelow hp)
    _ ≤ Real.exp (∑ p ∈ N.primesBelow,
        20 / (p : ℝ) ^ (3 / 2 : ℝ)) := by
      exact Real.prod_one_add_le_exp_sum _ fun p => by positivity
    _ ≤ maynardS2ReciprocalGCorrectionQuarterConstant := by
      unfold maynardS2ReciprocalGCorrectionQuarterConstant
      apply Real.exp_le_exp.mpr
      exact (maynardS2ReciprocalGCorrection_quarter_majorant_summable
        |>.sum_le_tsum (N.primesBelow) (fun n hn => by positivity))

private theorem maynardS2ReciprocalGCorrection_quarter_sum_range_le
    (W N : ℕ) :
    (∑ n ∈ Finset.range N,
        maynardS2ReciprocalGCorrectionQuarterTerm W n) ≤
      maynardS2ReciprocalGCorrectionQuarterConstant := by
  have hf1 : maynardS2ReciprocalGCorrectionQuarterTerm W 1 = 1 := by
    simp [maynardS2ReciprocalGCorrectionQuarterTerm,
      (maynardS2ReciprocalGCorrectionAF_isMultiplicative W).map_one]
  have hmul : ∀ {m n : ℕ}, Nat.Coprime m n →
      maynardS2ReciprocalGCorrectionQuarterTerm W (m * n) =
        maynardS2ReciprocalGCorrectionQuarterTerm W m *
          maynardS2ReciprocalGCorrectionQuarterTerm W n := by
    intro m n hmn
    exact maynardS2ReciprocalGCorrectionQuarterTerm_mul W hmn
  have hlocal : ∀ {p : ℕ}, p.Prime →
      Summable (fun i : ℕ =>
        ‖maynardS2ReciprocalGCorrectionQuarterTerm W (p ^ i)‖) := by
    intro p hp
  ...
  have hIndicator : Summable (N.smoothNumbers.indicator
      (maynardS2ReciprocalGCorrectionQuarterTerm W)) :=
  ...
  calc
    ...
    _ = ∑' n : N.smoothNumbers,
      ...
      (tsum_subtype N.smoothNumbers
        (maynardS2ReciprocalGCorrectionQuarterTerm W)).symm
    _ = ∏ p ∈ N.primesBelow,
        ∑' i : ℕ,
          maynardS2ReciprocalGCorrectionQuarterTerm W (p ^ i) :=
      hEuler.2.tsum_eq
    _ ≤ maynardS2ReciprocalGCorrectionQuarterConstant :=
      maynardS2ReciprocalGCorrection_local_quarter_product_le W N

theorem abs_maynardS2ReciprocalGCorrection_log_sum_range_le
    (W N : ℕ) :
    (∑ n ∈ Finset.range N,
      |maynardS2ReciprocalGCorrectionAF W n| * Real.log n) ≤
  ...
  calc
    (∑ n ∈ Finset.range N,
      ...
      calc
        ...
        _ = 4 * maynardS2ReciprocalGCorrectionQuarterTerm W n := by
          unfold maynardS2ReciprocalGCorrectionQuarterTerm
          ring
    _ = 4 * (∑ n ∈ Finset.range N,
        maynardS2ReciprocalGCorrectionQuarterTerm W n) := by
      rw [Finset.mul_sum]
    _ ≤ 4 * maynardS2ReciprocalGCorrectionQuarterConstant := by
      exact mul_le_mul_of_nonneg_left
        (maynardS2ReciprocalGCorrection_quarter_sum_range_le W N) (by norm_num)

theorem summable_abs_maynardS2ReciprocalGCorrection_log (W : ℕ) :
    Summable (fun n : ℕ =>
      |maynardS2ReciprocalGCorrectionAF W n| * Real.log n) := by
  apply summable_of_sum_range_le
  · intro n
    exact mul_nonneg (abs_nonneg _) (Real.log_natCast_nonneg n)
  · exact abs_maynardS2ReciprocalGCorrection_log_sum_range_le W

theorem tsum_abs_maynardS2ReciprocalGCorrection_log_le (W : ℕ) :
    (∑' n : ℕ,
      |maynardS2ReciprocalGCorrectionAF W n| * Real.log n) ≤
      4 * maynardS2ReciprocalGCorrectionQuarterConstant := by
  exact Real.tsum_le_of_sum_range_le
    (fun n => mul_nonneg (abs_nonneg _) (Real.log_natCast_nonneg n))
    (abs_maynardS2ReciprocalGCorrection_log_sum_range_le W)
