...
/-!
# Strong restricted S2 cross correction

The finite correction bound is divided by the exact exponent-106
coordinate-one scale. The supported-transform envelope retains the pre-sieve
density before division, the rough cross tail supplies the vanishing factor,
and the common reciprocal-`g` mass is normalized by the scalar mean limit.

This is the incompatible-correction part of SEM-396. The correction is the
project's exact finite CRT/Mobius correction, not the later pairwise-support
removal displayed as equation (6.17) in Maynard2013v3.
-/
...
theorem tendsto_engelsmaS2RestrictedCrossCorrection_div_squareScale_zero
  ...
  have henv : ∀ᶠ N : ℕ in atTop,
    ...
    have hEdivL : E N / L N =
      ...
      ring
    have hEdiv : E N / Q N =
        smallKCandidateBound * 8 * ((1 + L N) / L N) *
          (1 + (k : ℝ) * A N) := by
      calc
        E N / Q N = (E N / L N) / S N := by
          dsimp [Q]
          field_simp [ne_of_gt hLN, ne_of_gt hSN.1]
        _ = (smallKCandidateBound * 8 * S N * ((1 + L N) / L N) *
            (1 + (k : ℝ) * A N)) / S N := by rw [hEdivL]
        _ = smallKCandidateBound * 8 * ((1 + L N) / L N) *
          (1 + (k : ℝ) * A N) := by
          field_simp [ne_of_gt hSN.1]
  ...
  have hden : 0 < Q N ^ 106 := by positivity
  ...
  have hE0 : 0 ≤ E N / Q N := hEN.1
  have hsq : (E N / Q N) ^ 2 ≤ Cenv ^ 2 :=
  ...
  calc
    ...
    _ = (E N / Q N) ^ 2 * Tail N *
      ...
      have hk : k = 104 := by
        dsimp [k]
        simp [BoundedGaps.engelsmaTuple_card]
      rw [hk]
      field_simp [ne_of_gt hQN]
